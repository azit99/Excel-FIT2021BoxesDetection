...
def StackedHourglassNetwork(
    ...
    x = BatchNormalization(momentum=0.9)(x)
    x = LeakyReLU(alpha=0.1)(x)
    x = BottleneckBlock(x, input_shape[0] // 2, downsample=True)
    x = MaxPool2D(pool_size=2, strides=2)(x)
    x = BottleneckBlock(x, input_shape[0] // 2, downsample=False)
    x = BottleneckBlock(x, input_shape[0], downsample=True)

    ys = []
    for i in range(num_stack):
        x = HourglassModule(x, order=4, filters=input_shape[0], num_residual=num_residual)
        for i in range(num_residual):
            x = BottleneckBlock(x, input_shape[0], downsample=False)

        # predict 256 channels like a fully connected layer.
        x = LinearLayer(x, input_shape[0])
        ...
        if i < num_stack - 1:
            y_intermediate_1 = Conv2D(filters=input_shape[0], kernel_size=1, strides=1)(x)
            y_intermediate_2 = Conv2D(filters=input_shape[0], kernel_size=1, strides=1)(y)
...
if __name__ == "__main__":
    model= StackedHourglassNetwork(num_stack=1)
    print(model.summary())